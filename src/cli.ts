...
const orderProfiles = (
	...
	[...profiles].sort((a, b) => {
		const aDefault = defaultProfileId != null && a.id === defaultProfileId;
		const bDefault = defaultProfileId != null && b.id === defaultProfileId;
		...
		const byJournalName = a.journalName.localeCompare(b.journalName);
		if (byJournalName !== 0) {
			return byJournalName;
		}
		return a.id - b.id;
	});
...
const pickProfile = (
	...
	promptSelect(
		message,
		orderProfiles(profiles, defaultProfileId).map((profile) => ({
...
const matchesNetworkSearch = (
	network: (typeof CHAIN_OPTIONS)[number],
	...
	const searchable =
		`${network.name} ${network.key} ${network.chainId}`.toLowerCase();
	return normalizedQuery.split(" ").every((term) => searchable.includes(term));
};

const pickNetwork = (preferredNetwork?: string) =>
	Effect.gen(function* () {
		const alchemySupported = CHAIN_OPTIONS.filter((option) =>
			supportsAlchemyNetwork(option.key),
		);
		if (alchemySupported.length === 0) {
			return yield* Effect.fail(
				new Error("No Alchemy-supported networks are available."),
			);
		}

		const ordered = [...alchemySupported].sort((a, b) => {
		...
		while (true) {
			const matched = ordered.filter((network) =>
				matchesNetworkSearch(network, searchQuery),
			);
			if (matched.length > 0) {
				const message =
					searchQuery.length > 0
						? `Choose blockchain network (filtered: ${searchQuery})`
...
const pickTokenAddress = (
	...
	Effect.gen(function* () {
		...
		const orderedSuggestions = [...suggestions].sort((a, b) => {
			const aPreferred =
				existingLower != null && a.address.toLowerCase() === existingLower;
			const bPreferred =
				existingLower != null && b.address.toLowerCase() === existingLower;
			if (aPreferred && !bPreferred) {
				return -1;
			}
			if (!aPreferred && bPreferred) {
		...
		const knownExistingToken =
			existingLower != null &&
			orderedSuggestions.some(
				(suggestion) => suggestion.address.toLowerCase() === existingLower,
			);
		...
		const tokenChoice = yield* promptSelect("Choose ERC-20 token contract", [
			...
			...orderedSuggestions.map((suggestion) => ({
				...
				value: {
					tokenAddress: suggestion.address,
					tokenSymbol: suggestion.symbol as string | undefined,
				},
			})),
			{
				title: "Custom contract address",
				value: {
					tokenAddress: undefined as Address | undefined,
		...
		if (tokenChoice.tokenAddress) {
			const validatedTokenAddress = yield* RpcService.assertAddress(
		...
		const inputAddress = yield* promptRequired(
			"ERC-20 contract address (0x...)",
		);
		const tokenAddress = yield* RpcService.assertAddress(
			inputAddress,
			"ERC-20 address",
		);
...
const configureProfile = (
	existingProfile: NamedAppConfig | undefined,
	allProfiles: readonly NamedAppConfig[],
) =>
	Effect.gen(function* () {
		const odooUrl = yield* promptRequired(
			"Odoo URL",
			existingProfile?.odooUrl ?? process.env.ODOO_URL ?? "",
		);
		...
		if (companies.length === 0) {
			return yield* Effect.fail(
				new Error("No company exists in Odoo for this API key."),
			);
		}

		const sortedCompanies = [...companies].sort((a, b) => {
			const aPreferred =
				existingProfile?.companyId != null &&
				a.id === existingProfile.companyId;
			const bPreferred =
				existingProfile?.companyId != null &&
				b.id === existingProfile.companyId;
			if (aPreferred && !bPreferred) {
				return -1;
			}
			if (!aPreferred && bPreferred) {
		...
		const journals = yield* OdooService.listJournals(
			client,
			selectedCompany.id,
		);
		...
		if (journalMode === "existing") {
			if (journals.length === 0) {
				return yield* Effect.fail(
					new Error(
						`No journal exists in company "${selectedCompany.name}". Choose "Create a new journal".`,
					),
				);
			}
			...
			const sortedJournals = [...journals].sort((a, b) => {
				const aPreferred =
					existingProfile?.journalId != null &&
					a.id === existingProfile.journalId;
				const bPreferred =
					existingProfile?.journalId != null &&
					b.id === existingProfile.journalId;
				if (aPreferred && !bPreferred) {
					return -1;
				}
				if (!aPreferred && bPreferred) {
			...
			const name = yield* promptRequired(
				"New journal name",
				existingProfile?.journalName ?? "Crypto Transfers",
			);
			const code = yield* promptRequired(
				"New journal short code",
				toShortCode(name),
			);
			const currencyCode = yield* promptText(
				"Journal currency code (optional, e.g. USD/EUR)",
				"",
			);
			...
			yield* Console.log(
				`Created journal ${createdJournal.name} (id=${createdJournal.id}).`,
			);
		}

		const profileJournalName = journalName.trim();
		if (profileJournalName.length === 0) {
			return yield* Effect.fail(
				new Error(
					"Selected Odoo journal has an empty name. Set a journal name in Odoo first.",
				),
			);
		}
		...
		const profileId =
			existingProfile?.id ??
			allProfiles.reduce((maxId, profile) => {
				return profile.id > maxId ? profile.id : maxId;
			}, 0) + 1;

		const network = yield* pickNetwork(existingProfile?.network);
		const alchemyApiKey = yield* promptRequired(
			"Alchemy API Key (required)",
			existingProfile?.alchemyApiKey ?? process.env.ALCHEMY_API_KEY ?? "",
		).pipe(Effect.map((value) => value.trim()));
		const token = yield* pickTokenAddress(
			network,
			existingProfile?.tokenAddress,
		...
		const walletAddress = yield* promptRequired(
			...
			Effect.flatMap((value) =>
				RpcService.assertAddress(value, "wallet address"),
			),
		);
		...
		return {
			...
			journalId,
			journalName: profileJournalName,
			network,
			odooApiKey,
			odooUrl,
...
const setupProgram = Effect.gen(function* () {
	const store: AppConfigStore = (yield* loadConfigStoreOptional()) ?? {
		profiles: [],
	};
	...
	if (action === "remove") {
		const toRemove = yield* pickProfile(
			"Select config profile to remove",
			store.profiles,
			store.defaultProfileId,
		);
		const profiles = store.profiles.filter(
			(profile) => profile.id !== toRemove.id,
		);
		const nextDefaultProfileId =
			store.defaultProfileId === toRemove.id
				? profiles[0]?.id
				: (store.defaultProfileId ?? profiles[0]?.id);

		const nextStore: AppConfigStore = {
			profiles,
			...(nextDefaultProfileId != null
		...
		yield* Console.log(
			`\nRemoved profile id=${toRemove.id} (${toRemove.journalName}).`,
		);
		yield* Console.log(`File: ${configPath}`);
		if (profiles.length === 0) {
			yield* Console.log(
				"No config profiles left. Run `bun run setup` to add one.",
			);
		} else {
			yield* Console.log(`Remaining profiles: ${profiles.length}`);
			yield* Console.log(`Default profile id: ${nextStore.defaultProfileId}`);
		}
		return;
	}
	...
	const profileToEdit =
		action === "edit"
			? yield* pickProfile(
					"Select config profile to edit",
					store.profiles,
					store.defaultProfileId,
				)
			: undefined;

	const configuredProfile = yield* configureProfile(
		profileToEdit,
		store.profiles,
	);
	...
	const profiles =
		action === "edit" && profileToEdit
			? store.profiles.map((profile) =>
					profile.id === profileToEdit.id ? configuredProfile : profile,
				)
			: [...store.profiles, configuredProfile];

	const nextStore: AppConfigStore = {
		profiles,
		defaultProfileId: configuredProfile.id,
	};
	...
	yield* Console.log(`File: ${configPath}`);
	yield* Console.log(
		`Profile: ${configuredProfile.journalName} (id=${configuredProfile.id})`,
	);
	yield* Console.log(
		`Company: ${configuredProfile.companyName} (id=${configuredProfile.companyId})`,
	);
	yield* Console.log(
		`Journal: ${configuredProfile.journalName} (id=${configuredProfile.journalId})`,
	);
	yield* Console.log(
		`Network: ${getNetworkDisplayName(configuredProfile.network)} (${configuredProfile.network})`,
	);
	yield* Console.log("RPC provider: Alchemy");
...
const toStatementLine = (config: AppConfig, transfer: TransferRecord) => {
	...
	return {
		...
		payment_ref: explorerUrl ?? transfer.txHash,
		unique_import_id: transfer.uniqueId,
	};
};
...
const formatPlainLogMessage = (message: unknown): string => {
	...
	if (Array.isArray(message)) {
		return message.map(render).join(" ");
	}

	return render(message);
};

const nonVerboseLogger = Logger.withLeveledConsole(
	Logger.simple((message) => formatPlainLogMessage(message)),
);
const nonVerboseLoggerLayer = Layer.mergeAll(
	Logger.replace(Logger.defaultLogger, nonVerboseLogger),
	Logger.replace(Logger.prettyLoggerDefault, nonVerboseLogger),
);

const verboseOption = Options.boolean("verbose").pipe(
	Options.withAlias("v"),
	Options.withDescription(
		"Enable detailed sync logs (block ranges, chunk progress, and timing)",
	),
...
const syncProgram = (options: { verbose: boolean }) => {
	const program = Effect.gen(function* () {
		...
		if (store.profiles.length === 0) {
			return yield* Effect.fail(
				new Error(
					"No config profiles found. Run `bun run setup` and add at least one config profile.",
				),
			);
		}

		const config = yield* pickProfile(
			"Choose config profile for sync",
			store.profiles,
			store.defaultProfileId,
		);
		...
		if (!walletAddress) {
			return yield* Effect.fail(
				new Error(
					`Wallet address is required in profile id=${config.id} (${config.journalName}). Run \`bun run setup\`.`,
				),
			);
		}

		const fromDate = yield* promptIsoDate(
			"From date (YYYY-MM-DD)",
			defaultFromDate(),
		);
		const toDate = yield* promptIsoDate("To date (YYYY-MM-DD)", todayIso());
		const alchemyApiKey = config.alchemyApiKey.trim();
		if (alchemyApiKey.length === 0) {
			return yield* Effect.fail(
				new Error(
					`Alchemy API key is required in profile id=${config.id} (${config.journalName}). Run \`bun run setup\`.`,
				),
			);
		}
		...
		yield* Effect.logDebug(
			`Sync config: profileId=${config.id}, journal=${config.journalName}, network=${config.network}, token=${config.tokenAddress}, wallet=${walletAddress}, range=${fromDate}..${toDate}, rpcMode=alchemy`,
		);
		...
		if (options.verbose) {
			for (const [index, transfer] of transfers.entries()) {
				yield* Effect.logDebug(
					`Transfer key ${index + 1}/${transfers.length}: txHash=${transfer.txHash}, logIndex=${transfer.logIndex}, unique_import_id=${transfer.uniqueId}`,
				);
			}
		}

		yield* Effect.logInfo(
			`Found ${transfers.length} transfer(s). Preparing Odoo statement lines...`,
		);
		...
		const lines = transfers.map((transfer) =>
			toStatementLine(config, transfer),
		);
		const importIds = lines.map((line) => line.unique_import_id);

		yield* Effect.logDebug(
			`Checking ${importIds.length} import id(s) for deduplication in Odoo...`,
		);
		const existingImportIds = yield* OdooService.fetchExistingImportIds(
			client,
			importIds,
			{
				chunkSize: 100,
		...
		const newLines = lines.filter(
			(line) => !existingImportIds.has(line.unique_import_id),
		);
		yield* Effect.logDebug(
			`Dedup result: existing=${existingImportIds.size}, new=${newLines.length}`,
		);

		if (newLines.length === 0) {
			yield* Effect.logInfo(
				"All transfers already exist in Odoo (deduplicated by unique_import_id).",
			);
			return;
		}
		...
		yield* Effect.logDebug(
			`Writing ${newLines.length} statement line(s) in ${batches.length} batch(es)...`,
		);
		...
		for (const batch of batches) {
			...
			yield* Effect.logInfo(
				`Created ${created}/${newLines.length} statement lines...`,
			);
		}

		yield* Effect.logInfo("Sync completed.");
		yield* Effect.logInfo(`Profile: ${config.journalName} (id=${config.id})`);
		yield* Effect.logInfo(`Transfers found: ${transfers.length}`);
		yield* Effect.logInfo(
			`Already existing in Odoo: ${existingImportIds.size}`,
		);
